...
class RedisLocalBloomFilter(object):
    SLICE_KEY_FMT = "%s:%s"
    ...
    def __contains__(self, key):
        ...
        >>> b = RedisLocalBloomFilter(server=server, bfkeypreffix="atest:bf", capacity=100000, error_rate=0.001)
        >>> b.add("hello")
        False
        >>> "hello" in b
        True
    ...
    def copy(self):
        ...
        """
        raise NotImplementedError("RedisLocalBloomFilter not support copy")
...
both the same capacity and error rate")
        raise NotImplementedError("RedisLocalBloomFilter not support union")

    def __or__(self, other):
        raise NotImplementedError("RedisLocalBloomFilter not support or")
...
have equal capacity and error rate")
        raise NotImplementedError("RedisLocalBloomFilter not support intersection")

    def __and__(self, other):
        raise NotImplementedError("RedisLocalBloomFilter not support and")
    ...
    def fromfile(cls, f, n=-1):
        ...
        raise NotImplementedError("RedisLocalBloomFilter not support fromfile")
...
class ScalableRedisLocalBloomFilter(object):
    ...
    def __init__(self, server, bfkeypreffix, initial_capacity=100, error_rate=0.001,
        ...
        >>> b = ScalableRedisLocalBloomFilter(server=server, bfkeypreffix="satest:bf", initial_capacity=512, error_rate=0.001, \
                                    max_filters = 5,
                                    mode=ScalableRedisLocalBloomFilter.SMALL_SET_GROWTH)
        >>> b.add("test")
        False
        >>> "test" in b
        True
    ...
    def tofile(self, f):
        """Serialize this ScalableRedisLocalBloomFilter into the file-object
        `f'."""
